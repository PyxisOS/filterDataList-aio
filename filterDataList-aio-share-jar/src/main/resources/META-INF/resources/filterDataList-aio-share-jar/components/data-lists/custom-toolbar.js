...
(function()
{
   ...
   YAHOO.lang.augmentObject(Alvex.DataListsToolbar.prototype,
   {
      ...
      onReady: function DataListToolbar_onReady()
      {
        ...
        if( Dom.get( this.id + '-csvButton' ) )
        {
            this.widgets.csvButton = Alfresco.util.createYUIButton(this, "csvButton", this.onCsvExport,
            {
               disabled: false,
               value: "create"
            });
        ...
        if( Dom.get( this.id + '-pdfButton' ) )
        {
            this.widgets.pdfButton = Alfresco.util.createYUIButton(this, "pdfButton", this.onPdfExport,
            {
               disabled: false,
               value: "create"
            });
     ...
     onExcelExport: function()
      {
         ...
         var titleDatalist = this.modules.dataGrid.datalistMeta.title;
         var descriptionDatalist = this.modules.dataGrid.datalistMeta.description;
         
         var req = [];
         var row = [];
         var labels = [];
         for( var i = 0; i < headers.length; i++ )
            labels.push(headers[i].label);
         //req.push(row);
         ...
         var nomeFile =  titleDatalist +"."+ today.getFullYear() + '.' + (today.getMonth()+1) + '.' + today.getDate()
		...
		req.open("POST", Alfresco.constants.PROXY_URI + "export/data/excel/post", true); 
		//abilitazione CSRFPolicy
		if (Alfresco.util.CSRFPolicy && Alfresco.util.CSRFPolicy.isFilterEnabled())
            req.setRequestHeader( Alfresco.util.CSRFPolicy.getHeader(), Alfresco.util.CSRFPolicy.getToken() );
	 	req.setRequestHeader('Content-Type', 'application/json');    
	    req.responseType = "blob";     
		req.onload = function(e) {
		if (this.status === 200) {
			var blob = this.response;
				var filename = nomeFile + ".xlsx";
     ...
     onCsvExport: function()
      {
         ...
         var selecteditems = this.modules.dataGrid.getSelectedItems();
		...
		var req = new XMLHttpRequest();
	 	req.open("POST", Alfresco.constants.PROXY_URI + "export/data/csv/post", true);   
	 	//abilitazione CSRFPolicy
		if (Alfresco.util.CSRFPolicy && Alfresco.util.CSRFPolicy.isFilterEnabled())
            req.setRequestHeader( Alfresco.util.CSRFPolicy.getHeader(), Alfresco.util.CSRFPolicy.getToken());
	 	req.setRequestHeader('Content-Type', 'application/json');
		...
		req.onload = function(e) {
			if (this.status === 200) {
				var blob = this.response;
				   ...
				   console.log('Si è verifivato un errore nel download del file');
			   }
		   };
     ...
     onPdfExport: function()
      {
		...
		var req = new XMLHttpRequest();
	 	req.open("POST", Alfresco.constants.PROXY_URI + "export/data/pdf/post", true);    
	 	//abilitazione CSRFPolicy
		if (Alfresco.util.CSRFPolicy && Alfresco.util.CSRFPolicy.isFilterEnabled())
            req.setRequestHeader( Alfresco.util.CSRFPolicy.getHeader(), Alfresco.util.CSRFPolicy.getToken() );
	 	req.setRequestHeader('Content-Type', 'application/json'); 
