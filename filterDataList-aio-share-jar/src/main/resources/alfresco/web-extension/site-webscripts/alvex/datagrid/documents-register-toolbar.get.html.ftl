...
<div id="${args.htmlid}-body" class="datalist-toolbar toolbar">
   <div id="${args.htmlid}-headerBar" class="header-bar flat-button theme-bg-2">
      ...
      <div class="right" style="display: block;">
		...
		<span id="${id}-excelButton" class="yui-button yui-push-button excel-export">
		   <span class="first-child">
			  <button type="button">${msg('button.excel-export')}</button>
		   </span>
		</span>
		<span id="${id}-csvButton" class="yui-button yui-push-button csv-export">
		   <span class="first-child">
			  <button type="button">${msg('button.csv-export')}</button>
		   </span>
		</span>
		<span id="${id}-pdfButton" class="yui-button yui-push-button pdf-export">
		   <span class="first-child">
			  <button type="button">${msg('button.pdf-export')}</button>
		   </span>
		</span>
      </div>
   </div>
